...
#  it_processes_single_select_column "toDate(1234567890000)", 'c0', :date, Date.parse('2009-02-13').to_s, "toDate(1234567890000)"
  ...
  describe :column_value do
    ...
    context :no_time_zone do
      it "parses a string into a date when type is date" do
        @col.stub!(:type).and_return(:date)
        now = Date.new
        @executor.column_value(@col, now.to_s).should == now
      end
      
      it "returns a date when given a date when type is date" do
        @col.stub!(:type).and_return(:date)
        now = Date.new
        @executor.column_value(@col, now).should == now
      end
      
      it "parses a string into a time when type is datetime" do
        @col.stub!(:type).and_return(:datetime)
        now = Time.now
        @executor.column_value(@col, now.to_s).to_i.should == now.to_i
      end
      
      it "returns a time when given a time and type is datetime" do
        @col.stub!(:type).and_return(:datetime)
        now = Time.now
        @executor.column_value(@col, now).should == now
      end 
      
      it "parses a string into a time when type is timeofday" do
        @col.stub!(:type).and_return(:timeofday)
        now = Time.now
        @executor.column_value(@col, now.to_s).to_i.should == now.to_i
      end
      
      it "returns a time when given a time and type is timeofday" do
        @col.stub!(:type).and_return(:timeofday)
        now = Time.now
        @executor.column_value(@col, now).should == now
      end
    ...
    context :custom_time_zone do
      ...
      it "parses a string into a date when type is date" do  
        date = "March 08, 2012 17:00:00"
        @col.stub!(:type).and_return(:date)
        @executor.column_value(@col, date).should == Date.parse("March 08, 2012")
      end
      
      it "parses a string into a date when type is date and value is midnight" do
        date = "March 08, 2012 00:00:00"
        @col.stub!(:type).and_return(:date)
        @executor.column_value(@col, date).should == Date.parse("March 08, 2012")
      end
      ...
      it "parses a string into a time when type is datetime" do
        ...
        @col.stub!(:type).and_return(:datetime)
        parsed = @executor.column_value(@col, utc.strftime("%B %d, %Y %H:%M:%S"))
        (utc.to_i - parsed.to_i).should == Time.zone.utc_offset
      end
